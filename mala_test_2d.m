...
mcmc.mix_ratio = 1;   % Ratio of MALA steps to pCN steps (mixed method)
S = 1000000;      % MCMC samples
mcmc.beta = 0.01;       % pCN jump parameter
mcmc.h = 0.0001;                 % MALA step size parameter
mcmc.local = 1000;        % how many steps to calculate local acceptance rate?
...
%% Model parameters
md.d = 2;              % Spatial dimension 
md.H = [200,200,10];    % Layer widths
md.L = 3;              % Number of hidden layers
md.N = 2^5;            % Sample on N or N*N grid
md.J = 2^5;
md.gm = 0.1;
...
[X,Y] = meshgrid(linspace(-1,1,md.N));
md.XX = [X(:),Y(:)];

% Create the truth, forward map, data
UT = zeros(md.N);
UT((X+1).^2+0.5*(Y+1).^3 < 1^2) = 1;
UT((X-0.8).^2+(Y-0.8).^2 < 0.6^2) = 2;
ax = [0,2];

%UT = flipud(phantom('Modified Shepp-Logan',md.N));
UT = UT(:);

obsInd = round(linspace(1,md.N^2,md.J));
obsInd = randsample(md.N^2,md.J);
basis = speye(md.N^2);
...
subplot(131);
surf(X,Y,reshape(UT,md.N,md.N),'EdgeColor','None');view(2);axis square;
caxis(ax);
%hold on
...
for j=1:S
    ...
    ac = a0;
    xi = xi - ac*(DPhiXi + xi) + 0*sqrt(2*ac)*normrnd(0,1,md.NW,1);
...
UMean = zeros(md.N^2,1);
UVar = zeros(md.N^2,1);
xiMean = zeros(md.NW,1);
xiTrace = zeros(S,1);

xi = 0*normrnd(0,1,md.NW,1);
[~,PhiXi] = build_net(xi,md);
...
for s=1:S
    ...
    if strcmp(mcmc.method,'pcn')
        ...
        if mod(s,mcmc.local) == 0
            ...
            factor = max(1,2^((mcmc.burnin-s)/mcmc.burnin));
            if l_acc < mcmc.b_acc_target
                mcmc.beta = mcmc.beta/factor;
            else
                mcmc.beta = min(1,mcmc.beta*factor);
            end
        end
    ...
    elseif strcmp(mcmc.method,'mala')
        ...
        if mod(s,mcmc.local) == 0
            ...
            factor = max(1,2^((mcmc.burnin-s)/mcmc.burnin));
            if l_acc < 0.3
                mcmc.h = mcmc.h/factor;
            else
                mcmc.h = min(1,mcmc.h*factor);
            end
        end
    end
    ...
    if mod(s,mcmc.thinning) == 0           
        ...
        if s > mcmc.burnin
            ...
            subplot(133);
            surf(X,Y,reshape(UMean,md.N,md.N),'EdgeColor','None');view(2);axis square;
            caxis(ax);
        end
        
        subplot(132);
        surf(X,Y,reshape(U,md.N,md.N),'EdgeColor','None');view(2);axis square;
        caxis(ax);
        pause(0.01);
