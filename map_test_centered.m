% Optimization: try with Cauchy regularizer directly instead of noncentred
rng(69);
...
for j=1:S
    ...
    PhiTrace(j) = PhiXi+sum(log(1+xi.^2));
    DPhiTrace(j) = norm(DPhiXi + 2*xi./(1+xi.^2));
    ...
    ac = a0;
    xi = xi - ac*(DPhiXi + 2*xi./(1+xi.^2)) + 0*sqrt(2*ac)*normrnd(0,1,md.NW,1);
